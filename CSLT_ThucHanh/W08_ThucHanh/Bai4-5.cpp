...
void xoaTaiKhoanTimKiem(account* &ds, int &n){
    if (n == 0) {
        cout << "Danh sach rong" << endl;
        return;
    }

    char userxoa[MAX_LEN];
    cout << "Nhap username muon Xoa: ";
    cin.getline(userxoa, MAX_LEN);

    int index = -1;
    
    // tim vi tri
    for(int i = 0; i < n; i++){
        if(strcmp(ds[i].username, userxoa) == 0){
            index = i;
            break;
        }
    }
    ...
    // xoa
    if (index == -1){
        cout << ">> Khong tim thay tai khoan '" << userxoa << "' de xoa" << endl;;
    } 
    else {
        // copy
