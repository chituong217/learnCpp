...
void xuatTaiKhoanTimKiem(account* &ds, int &n){
    cout << "Nhap vao tai khoan muon tao: " << endl;
    account newAccount = nhapTaiKhoan();

    // bien check ton tai
    bool timthay = false;

    for (int i = 0; i < n; i++){
        if (strcmp(ds[i].username, newAccount.username) == 0) timthay = true;
    }

    if (timthay == false){
        // them tk vao cuoi ds neu ko tim thay

        account *newDS = new account[n + 1];
        for (int i = 0; i < n; i++){
            newDS[i] = ds[i];
        }
        newDS[n] = newAccount;
        ...
        // neu tim thay thi in ra tai khoan co cung username
        cout << "Tai khoan co trung username: " << endl;
        for (int i = 0; i < n; i++){
            if (strcmp(ds[i].username, newAccount.username) == 0){
                xuatTaiKhoan(ds[i]);
            }
        }
    }
